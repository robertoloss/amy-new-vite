...
function _requestObservable(client, httpRequest, options) {
  ...
  const request = new Observable((subscriber) => httpRequest(reqOptions, config2.requester).subscribe(subscriber));
  return options.signal ? request.pipe(_withAbortSignal(options.signal)) : request;
}
function _request(client, httpRequest, options) {
  const observable3 = _requestObservable(client, httpRequest, options).pipe(filter((event) => event.type === "response"), map((event) => event.body));
  return observable3;
}
...
function _listen(query, params) {
  ...
  return new Observable((observer) => {
    ...
    async function getEventSource() {
      const {
        default: EventSource
      } = await import("./browser-3JOSDQNW.js");
...
function defineCreateClientExports(envMiddleware2, ClassConstructor) {
  ...
  const createClient2 = (config2) => new ClassConstructor(defineHttpRequest(envMiddleware2, {
