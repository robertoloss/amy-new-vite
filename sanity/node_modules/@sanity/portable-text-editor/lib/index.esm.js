...
import { Element as Element$1, Text, Editor, Transforms, Node, Range, Operation, Path, createEditor } from 'slate';
...
const debug$i = debugWithName("applyPatches");
const debugVerbose$3 = debug$i.enabled && true;
...
const debugVerbose$2 = debug$h.enabled && false;
  ...
  return editor => {
    ...
    if (patches$) {
      editor.subscriptions.push(() => {
        ...
        const sub = patches$.subscribe(_ref3 => {
          let {
            patches,
            snapshot
          } = _ref3;
...
function transformOperation(editor, patch, operation, snapshot, previousSnapshot) {
  if (debugVerbose$2) {
    debug$h("Adjusting '".concat(operation.type, "' operation paths for '").concat(patch.type, "' patch"));
    debug$h("Operation ".concat(JSON.stringify(operation)));
    debug$h("Patch ".concat(JSON.stringify(patch)));
  }
  ...
  if (patch.type === "insert" && patch.path.length === 1) {
    ...
    debug$h("Adjusting block path (+".concat(patch.items.length, ") for '").concat(transformedOperation.type, "' operation and patch '").concat(patch.type, "'"));
  ...
  if (patch.type === "unset" && patch.path.length === 1) {
    ...
    if (debugVerbose$2) {
      debug$h("Selection ".concat(JSON.stringify(editor.selection)));
      debug$h("Adjusting block path (-1) for '".concat(transformedOperation.type, "' operation and patch '").concat(patch.type, "'"));
    }
            ...
            withoutSaving(editor, () => {
              withPreserveKeys(editor, () => {
                patches.forEach(patch => {
                  ...
                  if (debug$g.enabled) debug$g("Handling remote patch ".concat(JSON.stringify(patch)));
