...
function getCachedStringFieldPaths(type, maxDepth) {
  if (!type[stringFieldsSymbol]) {
    type[stringFieldsSymbol] = uniqBy__default.default([...BASE_WEIGHTS, ...deriveFromPreview(type), ...getStringFieldPaths(type, maxDepth).map(path => ({
      weight: 1,
      path
...
function getCachedBaseFieldPaths(type) {
  if (!type[stringFieldsSymbol]) {
    type[stringFieldsSymbol] = uniqBy__default.default([...BASE_WEIGHTS, ...deriveFromPreview(type)], spec => spec.path.join("."));
  }
  return type[stringFieldsSymbol];
}
